...
- 3 Interactive Tabs:
  1. Section Exam Schedules (Modality, Shift, Section selectors)
  2. Faculty Exam Timetables (Faculty dropdown & individual/all printable timetables)
  3. Faculty Roster & Assigned Subject Verification (Real-time search, Department pills, Subject checkmarks ✓)
- 120-minute High School & SHS Math exams span 2 continuous slots with '120 min.' duration badge
- Official AMIS circular logo in navbar and headers
- Corrected Exam Dates: Day 1 (Sep 2), Day 2 (Sep 3), Day 3 (Sep 6), Day 4 (Sep 7)
- Kindergarten 2 (1st Shift) starts at 1:30 PM
- Corrected Teacher Assignments strictly verified
- Dynamic break row colspan balancing for zero grid displacement
"""
...
# Build Comprehensive Faculty Roster Data
faculty_roster_list = []

for tid, tinfo in teacher_data.items():
    t_id = tinfo.get('id', tid)
    t_name = tinfo.get('canonical_name') or tinfo.get('name') or tid
    t_dept = tinfo.get('department', 'Faculty')
    ...
    for p in tinfo.get('periods', []):
        subj = p.get('subject')
        sec = p.get('section_name') or p.get('section')
        sh = p.get('shift', '')
        if subj and sec:
            key = (subj.upper(), sec.upper())
    ...
    # 2. From exam_data
    for e in exam_data:
        if e.get('teacher_id') == t_id:
            subj = e.get('subject')
            sec = e.get('section_name') or e.get('section')
            sh = e.get('shift', '')
            if subj and sec:
                key = (subj.upper(), sec.upper())
                if key not in seen_assign:
    ...
    t_exams = [e for e in exam_data if e.get('teacher_id') == t_id]
    
    faculty_roster_list.append({
        'id': t_id,
        'name': t_name,
        'department': t_dept,
        'assignments': sorted(assignments, key=lambda a: (a['subject'], a['section'])),
        'total_classes': tinfo.get('total_classes', len(assignments)),
        'total_exams': len(t_exams)
    })

faculty_roster_list = sorted(faculty_roster_list, key=lambda x: x['name'])
...
<title>AMIS — Official 1st Term Examination Schedule &amp; Faculty Roster</title>
...
<script src="class_schedules_data.js?v=20260819_0055"></script>
<script src="exam_data.js?v=20260819_0055"></script>
<script src="teacher_weekly_schedules.js?v=20260819_0055"></script>
...
<header class="top-toolbar">
  ...
  <div class="toolbar-controls" id="toolbarControls">
    <!-- Dynamic controls per active tab -->
  </div>
</header>

<!-- Interactive 3-Tab Bar -->
...
(function() {{
  ...
  const FACULTY_ROSTER_DATA = {json.dumps(faculty_roster_list, indent=2)};
  ...
  function switchTab(tabName, extraParam) {{
    currentTab = tabName;

    const btnSec = document.getElementById('tabBtnSection');
    const btnFac = document.getElementById('tabBtnFaculty');
    const btnRos = document.getElementById('tabBtnRoster');

    if (btnSec) btnSec.classList.toggle('active', tabName === 'SECTION');
    if (btnFac) btnFac.classList.toggle('active', tabName === 'FACULTY');
    if (btnRos) btnRos.classList.toggle('active', tabName === 'ROSTER');

    if (tabName === 'SECTION') {{
      renderSectionToolbar();
      renderSectionView();
    }} else if (tabName === 'FACULTY') {{
      renderFacultyToolbar(extraParam);
      renderFacultyView(extraParam || 'ALL_FACULTY');
  ...
  function renderSectionToolbar() {{
    toolbarControls.innerHTML = `
      ...
      <div class="filter-group">
        ...
        <select id="modalitySelect" class="filter-select" onchange="onModalityChange()">
      ...
      <div class="filter-group" id="shiftGroup" style="display:none;">
        ...
        <select id="shiftSelect" class="filter-select" onchange="onShiftChange()">
      ...
      <div class="filter-group">
        ...
        <select id="sectionSelect" class="filter-select" style="min-width:220px;" onchange="renderSectionView()">
          <option value="ALL_GROUP">All Filtered Sections</option>
        </select>
      </div>
  ...
  window.onModalityChange = function() {{
  ...
  function populateSectionDropdown() {{
    const mod = document.getElementById('modalitySelect') ? document.getElementById('modalitySelect').value : 'ALL';
    const shift = document.getElementById('shiftSelect') ? document.getElementById('shiftSelect').value : 'ALL';
  ...
  function renderSectionView() {{
    ...
    if (selectedSecId && selectedSecId !== 'ALL_GROUP') {{
  ...
  function renderFacultyToolbar(targetTeacherId) {{
    ...
    let optionsHtml = '<option value="ALL_FACULTY">All Staff / Faculty (Print All Timetables)</option>';
    teachersWithExams.forEach(t => {{
      const isSel = (targetTeacherId && t.id === targetTeacherId) ? 'selected' : '';
      optionsHtml += `<option value="${{t.id}}" ${{isSel}}>${{esc(t.name)}} (${{esc(t.department)}} • ${{t.total_exams}} Exams)</option>`;
    }});
  ...
  function renderFacultyView(selectedTid = 'ALL_FACULTY') {{
    ...
    targetTeachers.forEach(t => {{
      ...
      const teacherDept = t.department;
      const examDutyCount = t.total_exams;
...
print("✓ Successfully regenerated exam-schedule.html with 3 Interactive Tabs, including Faculty Roster & Assigned Subject Checkmarks!")
