...
- Multiple subjects appear in ONE ROW per exam day with duration-based grid spanning:
  * Base unit: 1 cell = 60 minutes
  * 60 minutes = span 1
  * 120 minutes = span 2 (e.g. HS Math occupies 2 cell widths in ONE ROW)
  * Total subjects counted as 1 per exam assignment (Math = 1 subject)
...
<script src="class_schedules_data.js?v=20260819_0015"></script>
<script src="exam_data.js?v=20260819_0015"></script>
<script src="teacher_weekly_schedules.js?v=20260819_0015"></script>
...
.timetable-grid thead th.col-day {
  width: 175px;
}

.timetable-grid thead th.col-window {
  width: 165px;
}
...
.timetable-grid tbody td.cell-day {
  font-weight: 800;
  font-size: 13px;
  color: var(--ink);
  background: #f8fafc;
...
.timetable-grid tbody td.cell-window {
  font-weight: 750;
  font-size: 12px;
  color: var(--ink-secondary);
  background: #f8fafc;
...
/* ONE ROW CSS Grid for Multiple Subjects */
.subject-row {
  display: grid;
  gap: 8px;
  width: 100%;
...
(function() {
  ...
  function renderTeacherView(teacherId) {
    ...
    targetTeacherIds.forEach(tid => {
      ...
      const examDutyCount = tExams.length; // True count of exam duties (Math is 1 duty)
            ...
            <table class="timetable-grid">
              <thead>
                <tr>
                  ...
                  <th class="col-window">Schedule Window</th>
                  <th>Exam Duty &amp; Duration Allocation (1 Visual Cell = 60 min.)</th>
                </tr>
              </thead>
              <tbody>
      `;
      ...
      EXAM_DATES.forEach(d => {
        const dayExams = tExams.filter(e => (e.day_number === d.day_num || e.short_date === d.short_date));
        dayExams.sort((a, b) => (a.slot_number || 0) - (b.slot_number || 0));

        if (dayExams.length === 0) {
          fullHtml += `
            <tr>
              <td class="cell-day">${esc(d.header)}</td>
              <td class="cell-window">--</td>
              <td style="color:#94a3b8; font-weight:600; font-size:12.5px; text-align:center; padding:18px;">
                No Examination Duty Scheduled
              </td>
            </tr>
          `;
        } else {
          const totalUnits = dayExams.reduce((sum, e) => sum + Math.max(1, Math.round((e.duration_minutes || 60) / 60)), 0);
          const firstSlot = dayExams[0].time_slot || dayExams[0].time || '';
          const lastSlot = dayExams[dayExams.length - 1].time_slot || dayExams[dayExams.length - 1].time || '';
          const startTime = firstSlot.includes('–') ? firstSlot.split('–')[0].trim() : firstSlot;
          const endTime = lastSlot.includes('–') ? lastSlot.split('–')[1].trim() : lastSlot;
          const windowStr = `${startTime} – ${endTime}`;

          fullHtml += `
            <tr>
              <td class="cell-day">${esc(d.header)}</td>
              <td class="cell-window">${esc(windowStr)}</td>
              <td style="padding: 8px 10px; vertical-align: middle;">
                <div class="subject-row" style="display: grid; grid-template-columns: repeat(${totalUnits}, 1fr); gap: 8px; width: 100%;">
                  ${dayExams.map(e => {
                    const units = Math.max(1, Math.round((e.duration_minutes || 60) / 60));
                    const color = getSubjectColor(e.subject);
                    const spanClass = units > 1 ? 'subject-' + (units * 60) : 'subject-60';
                    return `
                      <div class="subject-card ${spanClass}" style="grid-column: span ${units}; background:${color.bg}; border:1.5px solid ${color.border}; color:${color.text};">
                        <span class="cell-subject-sec">${esc(e.subject.toUpperCase())}</span>
                        <span class="cell-subtitle">${esc(cleanSectionName(e.section_name))}</span>
                        <span class="cell-duration-badge">${e.duration_minutes || 60} min.</span>
                      </div>
                    `;
                  }).join('')}
                </div>
  ...
  function renderSectionView() {
    ...
    targetSections.forEach(sec => {
            ...
            <table class="timetable-grid">
              <thead>
                <tr>
                  <th class="col-day">Day &amp; Date</th>
                  <th class="col-window">Schedule Window</th>
                  <th>Examination Subject Allocation (1 Visual Cell = 60 min.)</th>
                </tr>
              </thead>
              <tbody>
      `;
      ...
      EXAM_DATES.forEach(d => {
        const dayExams = secExams.filter(e => (e.day_number === d.day_num || e.short_date === d.short_date));
        dayExams.sort((a, b) => (a.slot_number || 0) - (b.slot_number || 0));
                ...
                <div class="subject-row" style="display: grid; grid-template-columns: repeat(${totalUnits}, 1fr); gap: 8px; width: 100%;">
                  ${dayExams.map(e => {
                    ...
                    return `
                      <div class="subject-card ${spanClass}" style="grid-column: span ${units}; background:${color.bg}; border:1.5px solid ${color.border}; color:${color.text};">
                        <span class="cell-subject-sec">${esc(e.subject.toUpperCase())}</span>
                        <span class="cell-subtitle">${esc(e.teacher)}</span>
                        <span class="cell-duration-badge">${e.duration_minutes || 60} min.</span>
                      </div>
                    `;
                  }).join('')}
...
print("✓ Successfully generated unified exam-schedule.html with One-Row Duration Grid layout & corrected dates!")
