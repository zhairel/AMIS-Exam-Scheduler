...
(function() {
  ...
  function populateTeacherDropdown() {
    // Count exam sessions per teacher from EXAM_RECORDS
  ...
  function getSectionExamScheduleModel(sec) {
    ...
    const secExams = EXAM_RECORDS.filter(e => e.section_name === secName || e.section === secName);
    ...
    if (shift === 'F2F') {
      timeline = [
        ...
        { is_break: false, time: '09:00 AM – 10:00 AM', minutes: '60 min.', slot_id: 's2' },
        ...
        { is_break: true, time: '11:25 AM – 12:45 PM', minutes: '80 min.', label: 'LUNCH & SALAH' },
        { is_break: true, time: '02:45 PM – 03:00 PM', minutes: '15 min.', label: 'SALAH & DISMISSAL' }
      ];
      ...
      if (isKinder) {
        ...
        timeline = [
          ...
          { is_break: true, time: '01:40 PM – 01:50 PM', minutes: '10 min.', label: 'TRANSITION' },
          { is_break: false, time: '01:50 PM – 02:50 PM', minutes: '60 min.', slot_id: 's2' },
          { is_break: true, time: '02:50 PM – 03:10 PM', minutes: '20 min.', label: 'BREAK / TRANSITION' },
          { is_break: false, time: '03:10 PM – 04:10 PM', minutes: '60 min.', slot_id: 's3' }
        ];
      }
    } else { // ODL - 2ND SHIFT
      timeline = [
        { is_break: true, time: '02:45 PM – 03:10 PM', minutes: '25 min.', label: 'HOMEROOM GUIDANCE / ASSEMBLY' },
        { is_break: false, time: '03:10 PM – 04:10 PM', minutes: '60 min.', slot_id: 's1' },
        { is_break: true, time: '04:10 PM – 04:20 PM', minutes: '10 min.', label: 'TRANSITION' },
        { is_break: false, time: '04:20 PM – 05:20 PM', minutes: '60 min.', slot_id: 's2' },
        ...
        <div class="timetable-sheet">
          <div class="school-header" style="display:flex; align-items:center; justify-content:center; gap:14px; margin-bottom:14px;">
            ...
            <div>
              ...
              <p style="margin:2px 0 0 0;">Official Examination Schedule S.Y. 2026–2027</p>
            </div>
          </div>
      ...
      timeline.forEach(row => {
        if (row.is_break) {
          ...
          EXAM_DATES.forEach(d => {
            // Find exam matching date and slot
            const exam = secExams.find(e => {
              ...
              // For 120m Math exams spanning slot 1 and slot 2
              if (e.duration_minutes >= 90 && (row.slot_id === 's1' || row.slot_id === 's2')) return true;
  ...
  function renderMultipleTeacherExamSheets(tIds) {
    ...
    tIds.forEach(tid => {
      ...
      // Extract unique time slots for this teacher
      const uniqueSlots = Array.from(new Set(tExams.map(e => e.time_slot || e.time))).sort();
        ...
        <div class="timetable-sheet">
          <div class="school-header" style="display:flex; align-items:center; justify-content:center; gap:14px; margin-bottom:14px;">
            ...
            <div>
              ...
              <p style="margin:2px 0 0 0;">1st Term Official Schedule S.Y. 2026–2027</p>
            </div>
          </div>
      ...
      if (uniqueSlots.length === 0) {
        fullHtml += `
          <tr>
            <td colspan="6" style="padding: 30px; text-align: center; color: #64748b; font-weight: 700; font-size: 13px;">
              No exam proctoring sessions scheduled for ${esc(teacherDisplayName)}.
            </td>
          </tr>
        `;
      } else {
        uniqueSlots.forEach(slotTime => {
          fullHtml += `
            <tr>
              <td class="cell-time">${esc(slotTime)}</td>
              <td class="cell-mins">60 min.</td>
          `;

          EXAM_DATES.forEach(d => {
            const matches = tExams.filter(e => (e.short_date === d.short_date || e.day_number === d.day_num) && (e.time_slot === slotTime || e.time === slotTime));
            ...
            if (matches.length === 1) {
              ...
              fullHtml += `
                <td class="cell-class" style="background:${color.bg}; border-color:${color.border}; color:${color.text};">
                  <div class="cell-class-inner">
                    <span class="cell-subject-sec">${esc(exam.subject)}</span>
                    <span class="cell-mod-badge" style="color:${color.text}; font-weight:800;">${esc(secShort)}</span>
                  </div>
                </td>
              `;
