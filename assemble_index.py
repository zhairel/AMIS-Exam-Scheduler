...
<head>
  ...
  <style>
    ...
    * {{
    ...
    .btn-poster-fullscreen:hover {{
      background: #f0fdf4;
      border-color: var(--brand-accent);
      transform: scale(1.05);
    }}

    .btn-poster-fullscreen svg {{
      width: 18px;
      height: 18px;
      fill: currentColor;
    }}

    .calendar-poster.is-fullscreen {{
...
<body>
  ...
  <script>
          ...
          <div class="calendar-poster" id="poster-${{grade.replace(/\\s+/g, '-')}}">
            <div class="poster-header">
              ...
              <div style="display:flex; align-items:center; gap:10px;">
                ...
                <button class="btn-poster-fullscreen" onclick="toggleFullscreenPoster(this)" title="Fullscreen Poster" aria-label="Toggle Fullscreen">
    ...
    window.toggleFullscreenPoster = function(btn) {{
      const poster = btn.closest('.calendar-poster');
      if (!poster) return;

      const isFs = poster.classList.toggle('is-fullscreen');
      ...
      if (isFs) {{
        ...
        if (poster.requestFullscreen && !document.fullscreenElement) {{
          poster.requestFullscreen().catch(() => {{}});
        }}
      }} else {{
        if (iconExpand) iconExpand.style.display = 'inline-block';
        if (iconCompress) iconCompress.style.display = 'none';
        btn.setAttribute('title', 'Fullscreen Poster');
    ...
    document.addEventListener('fullscreenchange', () => {{
      if (!document.fullscreenElement) {{
        document.querySelectorAll('.calendar-poster.is-fullscreen').forEach(p => {{
          p.classList.remove('is-fullscreen');
          const btn = p.querySelector('.btn-poster-fullscreen');
          if (btn) {{
            const iconExpand = btn.querySelector('.icon-expand');
            const iconCompress = btn.querySelector('.icon-compress');
            if (iconExpand) iconExpand.style.display = 'inline-block';
